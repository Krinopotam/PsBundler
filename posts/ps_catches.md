...
Когда функция обращается к переменной, то сначала PowserShell ищет среди локальных переменных. Если локальная переменная не найдена, то поиск ведется среди переменных уровня `script`, а за тем уровня `global`.
...
Для того, чтобы не попадать в подобные ситуации (да и в целом это правило хорошего тона в любом языке), никогда нельзя использовать неинициализированные переменные, а при обращении к переменным уровня `script` или `global` необходиямо явно указывать их scope.
...
Для того, чтобы замыкания реально работали, необходимо использовать метод `GetNewClosure()`. Исправим наш код:
...
## В PowerShell производительность может падать на базовых операциях ##

Например, простая конкатенация строк или классическое добавление элемента в массив используя выражение `+=` может существенно просадить производительность скрипта.

### Конкатенация строк ###

Все строки в PowerShell (как, кстати, и во многих других ЯП, например C#) являются иммутабельными. То есть при простой конкатенации `"A" + "B"` PowerShell на самом деле создает новую строку достаточно большой для хранения содержимого левых и правых операндов, а затем копирует элементы обоих операндов в новую строку.

На простых операциях это не заметно, но вот при многочисленных итерациях или при работе с длинными строками это существенно влияет на производительность.

В своей документации для повышениия производительности при работе со строками Microsoft рекомендует использовать 2 подхода:

- Хранить строки в массиве и объединять их оператором `-join`
- Использовать **.NET** класс `[StringBuilder]`

Microsoft даже приводит результаты измерений производительности. Например на 10 000 итераций объединения строк с нарастающим результатом:

- оператор `Join` — 14.75 мс
- класс `[StringBuilder]` — 62.44 мс (в 4.23 раза медленнее)
- обычная конкатенация `+=` — 619.64 мс (**в 42 раза медленнее**)

При 50 000 итераций результаты для обычной конкатенации становятся еще хуже:

- оператор `Join` — 43.15 мс
- класс `[StringBuilder]` — 304.32 мс (в 7 раз медленнее)
- обычная конкатенация `+=` — 14 225.13 мс (**в 330 раз медленнее**)

### Конкатенация массивов (добавление элементов в массив) ###

С массивами почти такая же история, что и со строками (и, кстати, строки в PowerShell это тоже массивы, только специального типа).


Подробнее можно посмотреть на сайте [Microsoft](https://learn.microsoft.com/ru-ru/powershell/scripting/dev-cross-plat/performance/script-authoring-considerations?view=powershell-5.1)
