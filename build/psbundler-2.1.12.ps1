...
class Replacer {
    ...
    [string]getModuleLoaderExpression([string]$moduleId, [string]$moduleName, [string]$reloadExpression) {
        $loaderVariableName = $this._config.modulesSourceMapVarName + '_GET_MODULE'
        return '(& $global:' + $loaderVariableName + ' "' + $moduleId + '" "' + $moduleName + '" ' + $reloadExpression + ')'
    }
...
class BundleBuilder {
    ...
    [string]getModuleRuntimeContent() {
        ...
        $cacheName = $sourceMapName + '_CACHE'
        $loadingName = $sourceMapName + '_LOADING'
        $loaderName = $sourceMapName + '_GET_MODULE'
        $newLine = [Environment]::NewLine

        $lines = @()
        $lines += '$global:' + $sourceMapName + ' = @{}'
        $lines += '$global:' + $cacheName + ' = @{}'
        $lines += '$global:' + $loadingName + ' = @{}'
        $lines += '$global:' + $loaderName + ' = {'
        $lines += '    param([string]$ModuleId, [string]$ModuleName, [bool]$Reload = $false)'
        $lines += ''
        $lines += '    if (-not $Reload -and $global:' + $cacheName + '.ContainsKey($ModuleId)) {'
        $lines += '        return $global:' + $cacheName + '[$ModuleId]'
        $lines += '    }'
        $lines += ''
        $lines += '    if ($global:' + $loadingName + '.ContainsKey($ModuleId)) {'
        ...
        $lines += '    }'
        $lines += ''
        $lines += '    $global:' + $loadingName + '[$ModuleId] = $true'
        $lines += '    try {'
        ...
        $lines += '        $global:' + $cacheName + '[$ModuleId] = $module'
        $lines += '        return $module'
        $lines += '    }'
        $lines += '    finally {'
        $lines += '        [void]$global:' + $loadingName + '.Remove($ModuleId)'
        $lines += '    }'
        $lines += '}'
...
$global:__PS_BUNDLER_MODULES_CACHE = @{}
$global:__PS_BUNDLER_MODULES_LOADING = @{}
$global:__PS_BUNDLER_MODULES_GET_MODULE = {
    param([string]$ModuleId, [string]$ModuleName, [bool]$Reload = $false)

    if (-not $Reload -and $global:__PS_BUNDLER_MODULES_CACHE.ContainsKey($ModuleId)) {
        return $global:__PS_BUNDLER_MODULES_CACHE[$ModuleId]
    }

    if ($global:__PS_BUNDLER_MODULES_LOADING.ContainsKey($ModuleId)) {
    ...
    $global:__PS_BUNDLER_MODULES_LOADING[$ModuleId] = $true
    try {
        ...
        $global:__PS_BUNDLER_MODULES_CACHE[$ModuleId] = $module
        return $module
    }
    finally {
        [void]$global:__PS_BUNDLER_MODULES_LOADING.Remove($ModuleId)
    }
}
...
Import-Module (& $global:__PS_BUNDLER_MODULES_GET_MODULE "src/PsBundler.psm1" "PsBundler" $true) -Force -DisableNameChecking
